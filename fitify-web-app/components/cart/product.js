...
const Product = ({item, decreaseAmount, increaseAmount, removeItem, order}) => {
    ...
    return(
        ...
        <div className=" flex flex-col justify-center ml-10 text-lg w-5/6">
            ...
            <p className="p-0 m-0 flex content-center sm:text-base text-sm"> Amount: 
                {!order && <button id={`decrease-button-${item.id}${item.size}`} className={`font-bold bg-fitify-purple mx-2 text-white rounded-2xl px-3 text-center ${!decreaseDisabled ? "hover:scale-105" : ""} disabled:opacity-60`} onClick={() => decreaseAmount(item.id,item.size)} > - </button>}  
                {item.amount}  
                {!order && <button id={`increase-button-${item.id}${item.size}`} className={`font-bold bg-fitify-purple mx-2 text-white rounded-2xl px-3 text-center ${increaseDisabled ? "hover:scale-105" : ""} disabled:opacity-60`} onClick={()=> increaseAmount(item.id,item.size)}>+</button>}
            </p>
            ...
            {!order && <button className=" bg-fitify-purple text-white py-2 px-2 sm:place-self-end place-self-start mt-3 sm:mr-8 mr-0 sm:text-base text-sm hover:opacity-60" onClick={() => removeItem(item.id, item.size)}>Remove</button>}
        </div>
        </div>
    );
}
