...
const CheckData = ({changeStep, user, setUser}) => {
    ...
    function changePage(e){
        e.preventDefault();
        console.log(!user.address || !user.city || !user.country);
    ...
    return(
        ...
        <form className="w-6/12 mx-auto flex flex-col items-start my-10">
            ...
            <input type="text" id="address" name="address" value={user?.address} onChange={(e) => handleChange(e)} className={`border-2 ${(!user.address && error) ? "border-fitify-pink" : "border-fitify-purple"} h-9 w-full sm:mx-0 mx-auto  form-field`}></input>
            <label htmlFor="country" className=" pr-3.2 mt-5 sm:w-full w-11/12 mx-auto ">*Country:</label>
            <input type="text" id="country" name="country" value={user?.country} onChange={(e) => handleChange(e)} className={`border-2 ${(!user.country && error) ? "border-fitify-pink" : "border-fitify-purple"} h-9 w-full sm:mx-0 mx-auto  form-field`}></input>
            <label htmlFor="city" className=" pr-3.2 mt-5 sm:w-full w-11/12 mx-auto ">*City:</label>
            <input type="text" id="city" name="city" value={user?.city} onChange={(e) => handleChange(e)} className={`border-2 ${(!user.city && error) ? "border-fitify-pink" : "border-fitify-purple"} h-9 w-full sm:mx-0 mx-auto  form-field`}></input>
            {error && <p>{error}</p>}
